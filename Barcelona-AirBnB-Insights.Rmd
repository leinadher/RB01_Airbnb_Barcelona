...
geojson_BCN$abbrev_neighbourhood <- str_trunc(geojson_BCN$neighbourhood,
                                             width = 18)

# Extract centroids to plot labels of neighbourhoods
...
m <- ggmap(map_BCN) +
  geom_sf(data = merged_data,
          ...
          linewidth = 0.3,
          size = 3) +
# POLYGON LABELS with repel to avoid overlap
  geom_label_repel(data = geojson_BCN,
                aes(label = abbrev_neighbourhood),
                size = 3,
                color = "black",
                box.padding = 0.5,
                point.padding = 0.1,
...
#
  scale_fill_gradient(low = alpha("white", 1), high = alpha("#f1796f", 1)) +
  theme_minimal() +
...
This plot provides some further insight into the distribution of the `price` variable that is not conveyed in the previous map plot. The `price` of listings is represented as points on a scatter plot, and clustered by `neighbourhood_group` in order to reflect the actual variation of values, and locate the *center* of the distributions. Because most listings' `price` values are located around the €100 range (median is €87), the items with much higher pricing significantly skew the plot. Thus, in order to improve the readability of the *median region*, the scale has been adjusted to be logarithmic. The additional violin plot lines provide more information about the distribution and location of medians.
...
The previous plot could be summarized in the above boxplot, which locates the quartiles and medians for the `price` variable in each neighbourhood group. As an example, the following are the medians from the neighbourhood groups mentioned in the previous plot, which represent the lowest and highest extremes in all 10:

```{r plot12.1, echo=TRUE, warning=FALSE, message=FALSE, error=FALSE}
...
This chart examines the distribution of listing counts based on the minimum nights required, aiming to illustrate this variable throughout the entire dataset. An initial observation can be made that the majority of listings require a minimum reservation of one night, followed by a rapid decline in frequency until reaching a full week. Further on, a significant upsurge in listings happens around the 30-day mark, indicating that a substantial cluster of listings entail a minimum stay of around a month.

Such findings lend support to the argument that Airbnb serves as a portal for an increasingly diverse range of rental types, extending beyond the original notion of short-term single-room leases, which were the original hallmark of the platform.

---

#### 2.3.14. Reviews boxplots by neighbourhood group
...
This set of boxplots looks at the distribution of number of reviews listings receive, grouped by neighbourhood groups. Although not as rich in information as other plots from the report, it does reflect certain trends which have been observed in other visualizations. Assuming that the number of reviews is a reflection of the level of activity it has received, once again the chart is dominated by the most popular neighbourhoods of *Eixample*, *Gràcia*, *Sant Martí* and *Sants-Montjuïc*. An exception to this observation is *Ciutat Vella*, which is shown to have a narrow boxplot, reflecting a smaller number of reviews. This may be do to the listings having a shorter lifespan, and not having been able to accumulate as many reviews, perhaps due to a more dynamic Airbnb market in that concetrated area. This statement would however have to be proven through other analyses.

---

#### 2.3.15. Correlation of numerical values

``` {r plot15, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, results='hide', fig.keep='all', fig.width=8, fig.height=8, fig.align='center'}
...
The above plot reflects the correlation of all numerical variables in the data set. A positive correlation is an indicator of a 'more is more' relationship between them, evaluated from 0 to 1. The interesting observation though is the fact that there are relatively low correlations between most variables. The single exception is the straightforward correlation between `reviews_per_month` and `number_of_reviews`, which is somewhat higher than 0.50. There are also variables shown to have no correlation at all, such as `star_rating` and `calculated_host_listings_count`, which seems also to be quite a reasonable conclusion.

---

#### 2.3.16. Price by star rating

```{r plot16, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, results='hide', fig.keep='all', fig.width=10, fig.height=5, fig.align='center'}
...
The last plot examines the behaviour of the `price` of all listings, plotted as scattered points, in relation to the `star_rating`, to explore possible correlations between the two. A general trend can be observed, that most listings tend to have high ratings, ranging from 2 to 5, forming a large cluster between the 4 and 5 marks. Also, the higher-priced listings are seen to be associated with higher star reviews. Outliers with nightly rates above €5000 are seen, skewing the perception of the overall trends of the main cluster.

The red line represents a linear regression model based on the two observations, which shows a positive linear increase in the price that is explained by the star rating. However, this causality cannot be proven only through this model. The linear model shows an intercept of 29.13, and a positive slope of 20.13 for the star_rating variable, indicating that for each increase star rating (1 star), the price would theoretically increase €20.13.

```{r plot16.1, echo=TRUE, warning=FALSE, message=FALSE, error=FALSE}
...
Among other libraries that weren not covered during the Bootcamp, we incorporated the `gridExtra` library to manipulate the `ggplot2` layout, allowing for efficient division and arrangement of layouts to optimize visualization.
...
The present study has been an great opportunity to get to know a project aimed at enhancing transparency within a company recognized for its limited data openness, as well as to explore data sets and workflows from official Open Data sources. Airbnb is in itself an interesting topic and controversial subject matter, as it has had a notable impact on the short-term rental and holiday accommodation sector across the world. The choice of Barcelona as the focal point is rooted in its reputation as a distinctive case study showcasing the influence of Airbnb on heavily frequented tourist destinations. Exploring this context is essential to achieving an understanding of the challenges posed by the platform. Furthermore, aligning the findings and statistical indicators with local laws and regulations has helped add a qualitative dimension to the assessment.

Our concise analysis reveals immediate concerns regarding the compliance of listings with legal requirements and highlights the emergence of a monopolistic market in the city. It also reflects economic and demographic discrepancies within the city, which could help lend support to studies in other knowledge areas. The observed levels of habitational stress in Barcelona, attributed in part to the phenomenon of 'overtourism' and a global housing crisis, are discernible within the Airbnb data. The dataset chosen for this project not only serves as a practical application of R code but also proves to be a source of rich and insightful information extending beyond a mere coding exercise.
